...
- Note: GPT Computer also offers a Hacking Edition based on Kali linux with modified prompts for cybersecurity tasks. The setup is the same as the regular version, just use the khulnasoft/gpt-computer:hacking image instead of khulnasoft/gpt-computer.

2.1. Pull the GPT Computer Docker image:
- Search for `khulnasoft/gpt-computer` in Docker Desktop
...
>
> ```bash
> docker pull khulnasoft/gpt-computer
> ```

2.2. OPTIONAL - Create a data directory for persistence:

> [!CAUTION]
> Preferred way of persisting GPT Computer data is to use the backup and restore feature.
> By mapping the whole `/a0` directory to a local directory, you will run into problems when upgrading GPT Computer to a newer version.
...
GPT Computer settings can be automatically configured using environment variables with the `GPTC_SET_` prefix in your `.env` file. This enables automated deployments without manual configuration.

**Usage:**
Add variables to your `.env` file in the format:
```
GPTC_SET_{setting_name}={value}
```

**Examples:**
```env
# Model configuration
GPTC_SET_chat_model_provider=anthropic
GPTC_SET_chat_model_name=claude-3-5-sonnet-20241022
GPTC_SET_chat_model_ctx_length=200000

# Memory settings
GPTC_SET_memory_recall_enabled=true
GPTC_SET_memory_recall_interval=5

# Agent configuration
GPTC_SET_agent_profile=custom
GPTC_SET_agent_memory_subdir=production
```
...
docker run -p 50080:80 \
  -e GPTC_SET_chat_model_provider=anthropic \
  -e GPTC_SET_chat_model_name=claude-3-5-sonnet-20241022 \
  khulnasoft/gpt-computer
```
...
- Click the `Run` button next to the `khulnasoft/gpt-computer` image
...
- OPTIONAL: Under "Volumes", configure your mapped folders, if needed:
  - Example host path: Your chosen directory (e.g., `C:\gpt-computer\memory`)
...
> Alternatively, run the following command in your terminal:
> ```bash
> docker run -p $PORT:80 -v /path/to/your/data:/a0 khulnasoft/gpt-computer
> ```
...
> [!CAUTION]
> **GitHub Copilot Provider:** When using the GitHub Copilot provider, after selecting the model and entering your first prompt, the OAuth login procedure will begin. You'll find the authentication code and link in the output logs. Complete the authentication process by following the provided link and entering the code, then you may continue using GPT Computer.
...
> [!NOTE]
> In settings, External Services tab, you can enable Cloudflare Tunnel to expose your GPT Computer instance to the internet.
...
> [!NOTE]
> Since v0.9, GPT Computer has a Backup and Restore feature, so you don't need to backup the files manually.
...
- GPT Computer will automatically detect your saved data and use it across memory, knowledge, skills, prompts and settings.
...
- Go to "Images" tab and remove the `khulnasoft/gpt-computer` image or click the three dots to pull the difference and update the Docker image.
...
> docker rmi khulnasoft/gpt-computer
>
