# Development manual for GPT Computer
This guide will show you how to setup a local development environment for GPT Computer in a VS Code compatible IDE, including proper debugger.
...
> [!NOTE]
> - GPT Computer runs in a Docker container, this simplifies installation and ensures unified environment and behavior across systems.
...
- GPT Computer is available on GitHub [github.com/khulnasoft/gpt-computer](https://github.com/khulnasoft/gpt-computer).
- You can download the files using a browser and extract or run `git clone https://github.com/khulnasoft/gpt-computer` in your desired directory.

> [!NOTE]
> In my case, I used `cd ~/Desktop` and `git clone https://github.com/khulnasoft/gpt-computer`, so my project folder is `~/Desktop/gpt-computer`.
...
- GPT Computer comes with `.vscode` folder that contains basic setup, recommended extensions, and debugger profiles. These will help us a lot.

1. Open your IDE and open the project folder using `File > Open Folder` and select your folder, in my case `~/Desktop/gpt-computer`.
...
## Step 4: Run GPT Computer in the IDE
Great work! Now you should be able to run GPT Computer from your IDE including real-time debugging.
...
After inserting my API key in settings, my GPT Computer instance works. I can send a simple message and get a response.
...
## Step 5: Run another instance of GPT Computer in Docker
- Some parts of GPTC require standardized linux environment, additional web services and preinstalled binaries that would be unneccessarily complex to set up in a local environment.
- To make development easier, we can use existing GPTC instance in docker and forward some requests to be executed there using SSH and RFC (Remote Function Call).

1. Pull the docker image `khulnasoft/gpt-computer` from Docker Hub and run it with a web port (`80`) mapped and SSH port (`22`) mapped.
...
This is how it looks in my example: port `80` is mapped to `8880` on the host and `22` to `8822`, `/a0` folder mapped to `/Users/frdel/Desktop/gpt-computer`:
...
You have successfully set up a complete GPT Computer development environment! You now have:
...
- ✅ The ability to develop, debug, and test GPT Computer features seamlessly

You're now ready to contribute to GPT Computer, create custom extensions, or modify the framework to suit your needs. Happy coding! 💻✨
...
For development and testing, you can override default settings using the `.env` file with `GPTC_SET_` prefixed variables:

```env
# Add to your .env file
GPTC_SET_chat_model_provider=ollama
GPTC_SET_chat_model_name=llama3.2
GPTC_SET_chat_model_api_base=http://localhost:11434
...
- Navigate to your project root in the terminal and run `docker build -f DockerfileLocal -t gpt-computer-local --build-arg CACHE_DATE=$(date +%Y-%m-%d:%H:%M:%S) .`
