# GPT Computer Backup/Restore Backend Specification

## Overview
This specification defines the backend implementation for GPT Computer's backup and restore functionality, providing users with the ability to backup and restore their GPT Computer configurations, data, and custom files using glob pattern-based selection. The backup functionality is implemented as a dedicated "backup" tab in the settings interface for easy access and organization.
...
- Integrates with GPT Computer's established error handling and toast notification system
...
backup_section: SettingsSection = {
    ...
    "description": "Backup and restore GPT Computer data and configurations using glob pattern-based file selection.",
    "fields": [
        {
            "id": "backup_create",
            "title": "Create Backup",
...
def _get_default_patterns(self) -> str:
    ...
    agent_root = self.gpt_computer_root.rstrip('/')
    user_home = self.user_home.rstrip('/')

    return f"""# GPT Computer Knowledge (excluding defaults)
{agent_root}/knowledge/**
!{agent_root}/knowledge/default/**

# GPT Computer Instruments (excluding defaults)
...
> **⚠️ CRITICAL FILE NOTICE**: The `{agent_root}/.env` file contains essential configuration including API keys, model settings, and runtime parameters. This file is **REQUIRED** for GPT Computer to function properly and should always be included in backups alongside `settings.json`. Without this file, restored GPT Computer instances will not have access to configured language models or external services.
...
class BackupCreate(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        patterns = input.get("patterns", "")
        include_hidden = input.get("include_hidden", False)
        backup_name = input.get("backup_name", "gpt-computer-backup")
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        ...
        try:
            ...
            return {
                ...
                "gpt_computer_version": metadata.get("gpt_computer_version", "unknown"),
...
The BackupService leverages GPT Computer's existing file operation helpers which already support RFC (Remote Function Call) routing for development mode. This ensures seamless operation whether running in direct mode or with container isolation.
...
class BackupService:
    """Core backup and restore service for GPT Computer"""

    def __init__(self):
        self.gpt_computer_version = self._get_gpt_computer_version()
        self.gpt_computer_root = files.get_abs_path("")  # Resolved GPT Computer root
    ...
    def _get_gpt_computer_version(self) -> str:
        """Get current GPT Computer version"""
        try:
    ...
    async def _get_environment_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "gpt_computer_root": files.get_abs_path(""),
    ...
    async def create_backup(self, patterns: str, include_hidden: bool = False, backup_name: str = "gpt-computer-backup") -> str:
        ...
        try:
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                metadata = {
                    ...
                    "gpt_computer_version": self.gpt_computer_version,
    ...
    def create_backup_with_progress(self, patterns: str, include_hidden: bool = False, backup_name: str = "gpt-computer-backup"):
...
#### GPT Computer Internal Dependencies
The backup system requires these GPT Computer helper modules:
...
#### Integration with GPT Computer Error System
The backup system integrates with GPT Computer's existing error handling infrastructure:

```python
from python.helpers.errors import format_error
from python.helpers.print_style import PrintStyle

# Follow GPT Computer's error handling patterns
try:
...
GPT Computer's existing task scheduler could be extended to support automated backups:
...
The backup system uses the same version detection method as GPT Computer's main UI:
...
- **Environment Details**: User, timezone, working directory, runtime mode, GPT Computer root path
...
- **Compatibility Data**: GPT Computer version and environment for restoration validation
...
This enhanced backend specification provides a production-ready, comprehensive backup and restore system with advanced metadata tracking, real-time progress monitoring, and intelligent file management capabilities, all while maintaining GPT Computer's architectural patterns and security standards.
...
- **Testing Verified**: BackupService initializes correctly and detects GPT Computer root paths
...
**Result:** Critical files like `.env` are now properly backed up when explicitly specified, ensuring GPT Computer configurations are preserved.
...
The GPT Computer backup system is now production-ready and battle-tested! 🚀
...
backed_up_agent_root = environment_info.get("gpt_computer_root", "")
```
...
- Works seamlessly between different GPT Computer installations
...
The GPT Computer backup system now provides:
