...
import gpt_computer.helpers.log as Log
...
from gpt_computer.helpers.extension import call_extensions
from gpt_computer.helpers.errors import RepairableException
...
class AgentContext:
    ...
    def __init__(
        ...
        self.gptc = gptc or Agent(0, self.config, self)
    ...
    def get_notification_manager(cls):
        if cls._notification_manager is None:
            from gpt_computer.helpers.notification import NotificationManager  # type: ignore
    ...
    def reset(self):
        self.kill_process()
        self.log.reset()
        self.gptc = Agent(0, self.config, self)
    ...
    def nudge(self):
        self.kill_process()
        self.paused = False
        self.task = self.communicate(UserMessage(self.gptc.read_prompt("fw.msg_nudge.md")))
        return self.task

    def get_agent(self):
        return self.streaming_agent or self.gptc
...
class Agent:
    ...
    def __init__(
        ...
        # agent context
        self.context = context or AgentContext(config=config, gptc=self)
    ...
    async def process_tools(self, msg: str):
        ...
        if tool_request is not None:
            ...
            try:
                import gpt_computer.helpers.mcp_handler as mcp_helper
    ...
    def get_tool(
        ...
        **kwargs,
        ...
        from gpt_computertools.unknown import Unknown
        from gpt_computer.helpers.tool import Tool

        classes = []

        # search for tools in agent's folder hierarchy
        paths = subagents.get_paths(self, "tools", name + ".py", default_root="gpt_computer")
        for path in paths:
            try:
