...
//     if (out.name.startsWith("data-") && out.name != "data-GPTCUID" && out.name != "data-gptc-frame-id") return null;
...
//   window.__GPTC_extractFrameContent = function() {
...
//           node.setAttribute("data-GPTCUID", uid);
//         }
...
//         if (tagName) {
//           content += ` selector="${node.getAttribute("data-GPTCUID")}"`;
//         }
...
//     if (event.data === 'GPTC_REQUEST_CONTENT') {
//       // Extract content and send it back to parent
//       const content = window.__GPTC_extractFrameContent();
//       // Use '*' as targetOrigin since we're in a controlled environment
//       window.parent.postMessage({
//         type: 'GPTC_FRAME_CONTENT',
//         content: content,
//         frameId: window.frameElement?.getAttribute('data-gptc-frame-id')
//       }, '*');
//     }
//   });

//   // Function to extract content from all frames
//   window.__GPTC_extractAllFramesContent = async function(rootNode = document) {
//     let content = "";
    
//     // Extract content from current document
//     content += window.__GPTC_extractFrameContent();
...
//         iframe.setAttribute('data-gptc-frame-id', frameId);
        
//         // Setup one-time message listener for this specific frame
//         const listener = function(event) {
//           if (event.data?.type === 'GPTC_FRAME_CONTENT' && 
...
//         iframe.contentWindow.postMessage('GPTC_REQUEST_CONTENT', '*');
...
//       const frameId = iframe.getAttribute('data-gptc-frame-id');
