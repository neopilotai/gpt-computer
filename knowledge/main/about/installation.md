...
Click to open a video to learn how to install GPT Computer:

[![Easy Installation guide](/docs/res/easy_ins_vid.png)](https://www.youtube.com/watch?v=w5v5Kjx51hs)

The following user guide provides instructions for installing and running GPT Computer using Docker, which is the primary runtime environment for the framework. For developers and contributors, we also provide instructions for setting up the [full development environment](#in-depth-guide-for-full-binaries-installation).
...
- Docker Desktop provides the runtime environment for GPT Computer, ensuring consistent behavior and security across platforms
...
- Note: GPT Computer also offers a Hacking Edition based on Kali linux with modified prompts for cybersecurity tasks. The setup is the same as the regular version, just use the xeondesk/gpt-computer:hacking image instead of xeondesk/gpt-computer.

2.1. Pull the GPT Computer Docker image:
- Search for `xeondesk/gpt-computer` in Docker Desktop
...
>
> ```bash
> docker pull xeondesk/gpt-computer
> ```

2.2. Create a data directory for persistence:
- Choose or create a directory on your machine where you want to store GPT Computer's data
- This can be any location you prefer (e.g., `C:/gpt-computer-data` or `/home/user/gpt-computer-data`)
- This directory will contain all your GPT Computer files, like the legacy root folder structure:
  ...
  - `.env` - Your API keys
  - `settings.json` - Your GPT Computer settings

> [!TIP]
> Choose a location that's easy to access and backup. All your GPT Computer data
> will be directly accessible in this directory.

2.3. Run the container:
- In Docker Desktop, go back to the "Images" tab
- Click the `Run` button next to the `xeondesk/gpt-computer` image
...
- Under "Volumes", configure:
  - Host path: Your chosen directory (e.g., `C:\gpt-computer-data`)
  - Container path: `/gptc`
...
> Alternatively, run the following command in your terminal:
> ```bash
> docker run -p $PORT:80 -v /path/to/your/data:/gptc xeondesk/gpt-computer
> ```
...
> [!NOTE]
> After starting the container, you'll find all GPT Computer files in your chosen
...
3. Configure GPT Computer
- Refer to the following sections for a full guide on how to configure GPT Computer.

## Settings Configuration
GPT Computer provides a comprehensive settings interface to customize various aspects of its functionality. Access the settings by clicking the "Settings"button with a gear icon in the sidebar.
...
Learn more about Remote Function Calls and their purpose [here](#7-configure-gpt-computer-rfc).

> [!IMPORTANT]
> Always keep your API keys and passwords secure.

# Choosing Your LLMs
The Settings page is the control center for selecting the Large Language Models (LLMs) that power GPT Computer.  You can choose different LLMs for different roles:

| LLM Role | Description |
| --- | --- |
| `chat_llm` | This is the primary LLM used for conversations and generating responses. |
| `utility_llm` | This LLM handles internal tasks like summarizing messages, managing memory, and processing internal prompts.  Using a smaller, less expensive model here can improve efficiency. |
| `embedding_llm` | This LLM is responsible for generating embeddings used for memory retrieval and knowledge base lookups. Changing the `embedding_llm` will re-index all of GPTC's memory. |
...
> mixed in the vector database. Note that this will DELETE ALL of GPT Computer's memory.
...
#### Selecting your model within GPT Computer
...
## Using GPT Computer on your mobile device
GPT Computer's Web UI is accessible from any device on your network through the Docker container:
...
> [!NOTE]
> If you're running GPT Computer directly on your system (legacy approach) instead of
> using Docker, you'll need to configure the host manually in `run_ui.py` to run on all interfaces using `host="0.0.0.0"`.

For developers or users who need to run GPT Computer directly on their system,see the [In-Depth Guide for Full Binaries Installation](#in-depth-guide-for-full-binaries-installation).

# How to update GPT Computer

1. **If you come from the previous version of GPT Computer:**
- Your data is safely stored across various directories and files inside the GPT Computer folder.
- To update to the new Docker runtime version, you might want to backup the following files and directories:
  ...
  - `/tmp/settings.json` - Your GPT Computer settings
...
- Reach for the folder where you saved your data and copy it to the new GPT Computer folder set during the installation process.
- GPT Computer will automatically detect your saved data and use it across memory, knowledge, instruments, prompts and settings.

> [!IMPORTANT]
> If you have issues loading your settings, you can try to delete the `/tmp/settings.json` file and let GPT Computer generate a new one.
...
- Go to "Images" tab and remove the `xeondesk/gpt-computer` image or click the three dots to pull the difference and update the Docker image.
...
> your GPT Computer data directory (the chosen directory on your machine).
...
> docker stop gpt-computer
>
> # Remove the container (data is safe in the folder)
> docker rm gpt-computer
>
> # Remove the old image
> docker rmi xeondesk/gpt-computer
>
...
3. **Full Binaries**
- Using Git/GitHub: Pull the latest version of the GPT Computer repository.
...
> - If any changes are made to the requirements of the updated version, you have to execute this command inside the gpt conda env after activating it:
> ```bash
> pip install -r requirements.txt

# In-Depth Guide for Full Binaries Installation
- GPT Computer is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make GPT Computer work with its full development environment.
...
2. You don't necessarily need API keys: GPT Computer can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running GPT Computer without Docker makes the process more complicated and it's thought for developers and contributors.
...
## 2. Download GPT Computer
- You can clone the GPT Computer repository (https://github.com/xeondesk/gpt-computer) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the GPT Computer releases [here](https://github.com/xeondesk/gpt-computer/releases).
2. The latest release is on the top of the list, click the "Source Code (zip)" button under "Assets" to download it.

<img src="res/setup/image-14-u.png" alt="gpt computer download" width="500"/>
<br><br>

3. Extract the downloaded archive where you want to have it. I will extract them to "gpt-computer" folder on my Desktop - "C:\Users\frdel\Desktop\gpt-computer" on Windows and "/Users/frdel/Desktop/gpt-computer" on macOS.
...
2. In the terminal, navigate to your GPT Computer folder using **"cd"** command. Replace the path with your actual GPT Computer folder path.
~~~
cd C:\Users\frdel\Desktop\gpt-computer
~~~
You should see your folder has changed on the next terminal line.

<img src="res/setup/image-15.png" alt="gpt computer cd" height="100"/>
...
conda activate a0
~~~
And you should see that the **(base)** on the left has changed to **(gptc)**. This means that this terminal now uses the new **a0** virtual environment and all packages will be installed into this environment.
...
Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox GPT Computer.
- GPT Computer only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
...
> Docker socket to be used (requires password)."  This allows GPT Computer to
...
- GPT Computer needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/gptc` volume to the location of your local GPT Computer folder.
- Run the Docker container following the instructions.

## 5. Run the local GPT Computer instance
Run the GPT Computer with Web UI:
~~~
python run_ui.py
~~~
...
- Open the URL shown in terminal in your web browser. You should see the GPT Computer interface.

## 6. Configure GPT Computer
Now we can configure GPT Computer - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure GPT Computer.

## 7. Configure GPT Computer RFC
GPT Computer needs to be configured further to redirect some functions to the Docker container. This is crucial for development as GPTC needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on GPT Computer.
...
### Conclusion
After following the instructions for your specific operating system, you should have GPT Computer successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents.

If you encounter any issues during the installation process, please consult the [Troubleshooting section](troubleshooting.md) of this documentation or refer to the GPT Computer [Skool](https://www.skool.com/gpt-computer) or [Discord](https://discord.gg/B8KZKNsPpj) community for assistance.
