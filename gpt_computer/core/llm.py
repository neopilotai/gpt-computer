...
from gpt_computer.helpers import browser_use_monkeypatch, dirty_json, dotenv, settings
from gpt_computer.helpers.dotenv import load_dotenv
from gpt_computer.helpers.providers import (
    ModelType as ProviderModelType,
    get_provider_config,
)
from gpt_computer.helpers.rate_limiter import RateLimiter
from gpt_computer.helpers.tokens import approximate_tokens
    ...
    def _process_thinking_tags(self, response: str, reasoning: str) -> ChatChunk:
        if self.thinking:
            ...
            if close_pos != -1:
                ...
                self.thinking = False
                self.thinking_tag = ""
                # recursively process the rest of the response for more tags
                return self._process_thinking_tags(response, reasoning)
            else:
                if self._is_partial_closing_tag(response):
                    # find where the partial tag starts
                    for i in range(1, len(self.thinking_tag)):
                        if response.endswith(self.thinking_tag[:i]):
                            self.unprocessed = self.thinking_tag[:i]
                            reasoning += response[:-i]
                            return ChatChunk(response_delta="", reasoning_delta=reasoning)
            ...
            for opening_tag, closing_tag in self.thinking_pairs:
                pos = response.find(opening_tag)
                if pos != -1 and (earliest_pos == -1 or pos < earliest_pos):
                    earliest_pos = pos
                    found_opening = opening_tag
                    found_closing = closing_tag

            if earliest_pos != -1:
                # Add text before the tag to response
                prefix = response[:earliest_pos]
                response = response[earliest_pos + len(found_opening):]
                self.thinking = True
                self.thinking_tag = found_closing

                # Process the rest
                inner_res = self._process_thinking_tags(response, reasoning)
                return ChatChunk(
                    response_delta=prefix + inner_res["response_delta"],
                    reasoning_delta=inner_res["reasoning_delta"]
                )
            else:
                # Check for partial opening tags at the end of response
                for opening_tag, closing_tag in self.thinking_pairs:
                    for i in range(len(opening_tag) - 1, 0, -1):
                        if response.endswith(opening_tag[:i]):
                            self.unprocessed = opening_tag[:i]
                            return ChatChunk(response_delta=response[:-i], reasoning_delta=reasoning)
...
class LiteLLMChatWrapper(SimpleChatModel):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Set GPTC model config as instance attribute after parent init
        self.gptc_model_conf = model_config
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        apply_rate_limiter_sync(self.gptc_model_conf, str(msgs))
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        # Apply rate limiting if configured
        await apply_rate_limiter(self.gptc_model_conf, str(msgs))
    ...
    async def unified_call(
        ...
        **kwargs: Any,
        ...
        limiter = await apply_rate_limiter(
            self.gptc_model_conf, str(msgs_conv), rate_limiter_callback
        )

        # Prepare call kwargs and retry config (strip GPTC-only params before calling LiteLLM)
        call_kwargs: dict[str, Any] = {**self.kwargs, **kwargs}
        max_retries: int = int(call_kwargs.pop("gptc_retry_attempts", 2))
        retry_delay_s: float = float(call_kwargs.pop("gptc_retry_delay_seconds", 1.5))
...
class BrowserCompatibleChatWrapper(ChatOpenRouter):
    ...
    async def _acall(
        ...
        **kwargs: Any,
        ...
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self._wrapper.gptc_model_conf, str(messages))
...
class LiteLLMEmbeddingWrapper(Embeddings):
    model_name: str
    kwargs: dict = {}
    gptc_model_conf: Optional[ModelConfig] = None
    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self.gptc_model_conf, " ".join(texts))
    ...
    def embed_query(self, text: str) -> List[float]:
        # Apply rate limiting if configured
        apply_rate_limiter_sync(self.gptc_model_conf, text)
...
def _adjust_call_args(provider_name: str, model_name: str, kwargs: dict):
    ...
    if provider_name == "openrouter":
        kwargs["extra_headers"] = {
            "HTTP-Referer": "https://gpt.khulnasoft.com",
            "X-Title": "GPT Computer",
        }
