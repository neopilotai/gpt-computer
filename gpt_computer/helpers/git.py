...
# Files to ignore when checking dirty status (GPTC project metadata)
GPTC_IGNORE_PATTERNS = {".a0proj", ".a0proj/"}


def get_repo_status(repo_path: str) -> dict:
    """Get Git repository status, ignoring GPTC project metadata files."""
    try:
        repo = Repo(repo_path)
        if repo.bare:
        ...
        # Check dirty status, excluding GPTC metadata
        def is_gptc_file(path: str) -> bool:
            return path.startswith(".a0proj") or path == ".a0proj"

        # Filter out GPTC files from diff and untracked
        changed_files = [d.a_path for d in repo.index.diff(None)] + [d.a_path for d in repo.index.diff("HEAD")]
        untracked = repo.untracked_files

        real_changes = [f for f in changed_files if not is_gptc_file(f)]
        real_untracked = [f for f in untracked if not is_gptc_file(f)]
